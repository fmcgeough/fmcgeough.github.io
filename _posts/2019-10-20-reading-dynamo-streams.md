...
AWS provides a key-value store called DynamoDB. Its got several nice features that make it a popular choice
for companies using AWS. Its self-managed and can scale based on usage patterns, for example. One of its
other nice features is that there is a stream API that gives you access to changes that have been made over
the last 24 hours. For this blog post I'll build a little github repo that demonstrates that stream API.

Another nice feature of DynamoDB is that you can use docker to run a local copy of DynamoDB. I'm a big fan of this. If
only all the AWS services provided some means of development and testing without using AWS itself. In any case,
it'll allow you to download the repo I build in github and try it out without even having an AWS account. Here's
the docker commands you can use to get a local DynamoDB running:
