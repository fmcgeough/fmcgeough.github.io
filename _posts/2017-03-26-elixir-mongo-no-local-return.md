...
tags: []
---
[Mongodb](https://hex.pm/packages/mongodb) library uses the [dbconnection](https://hex.pm/packages/db_connection) library,
...
So, yes. The call into the Erlang hash function will always return 0. But if the code just had "raise message" then
dialyzer would not be happy since at that point there would be no possibility that the function could ever return
a value and that's a dialyzer no-no. Since the library developers wanted dialyzer to run cleanly they decided to use this
little trick and then provided a bit of documentation in the "unused" return value. This function should never be called. It is supposed to implemented by the protocol for the particular datasource you are using (Postgresql, Mongo, or whatever).
